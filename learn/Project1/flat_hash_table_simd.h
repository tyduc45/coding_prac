...
namespace mini_absl {
	...
	class Group {
	public:
		explicit Group(const ctrl_t* pos): ctrl_(_mm_loadu_si128(reinterpret_cast<const __m128i*>(pos))){}
		// 返回一个位掩码: 第 i 位为 1 表示组内第 i 个槽的 H2 匹配
		uint32_t Match(ctrl_t h2) const
		{
			auto target = _mm_set1_epi8(h2);
			return _mm_movemask_epi8(_mm_cmpeq_epi8(target, ctrl_));
		}
		// 返回一个位掩码: 第 i 位为 1 表示该槽为空
		uint32_t MatchEmpty() const
		{
			auto target = _mm_set1_epi8(kEmpty);
			return _mm_movemask_epi8(_mm_cmpeq_epi8(target, ctrl_));
		}
		// 返回一个位掩码: 第 i 位为 1 表示该槽为空或已删除(可用于插入)
		uint32_t MatchEmptyOrDeleted() const {
			auto empty	 = _mm_cmpeq_epi8(ctrl_, _mm_set1_epi8(kEmpty));
			auto deleted = _mm_cmpeq_epi8(ctrl_, _mm_set1_epi8(kDeleted));
			return _mm_movemask_epi8(_mm_or_si128(empty, deleted));
		}
